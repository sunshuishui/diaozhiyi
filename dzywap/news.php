...
include './common.php';
/*$sql='select id from article where type=1';
$rs=$db->query($sql);
...
$sql="select * from article where type=1 order by addtime desc,id desc limit $thispage,$pagesize";*/
$sql="select * from article where type=1 order by addtime desc,id desc";
$rs=$db->query($sql);
while($row=$db->fetch_array($rs)){
	$row['content']=strip_tags($row['content']);
	$article[]=$row;	
}
$smarty->assign('hovernav','news');
