<?php
class Xiangmumodel extends CI_Model{
	public function alist($tiaoguo,$pagesize){
		if($pagesize){
			$rs=$this->db->order_by('id desc')->limit($pagesize,$tiaoguo)->get('xiangmu');
		}else{
			$rs=$this->db->select('id')->get('xiangmu');
		}
	...
	public function addsave($pic){
		...
		$bool=$this->db->insert('xiangmu',$data);
		return $bool;
	}
	public function singleinfo($id){
		$rs=$this->db->where(array('id'=>$id))->get('xiangmu');
		return $rs->row_array();
	}
	public function delete($id){
		...
		return $this->db->delete('xiangmu',array('id'=>$id));
	}
	public function editsave($pic){
		...
		$bool=$this->db->where(array('id'=>$id))->update('xiangmu',$data);
		return $bool;
	}
}